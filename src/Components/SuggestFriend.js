...
export default function SuggestFriend({ navigation, data, updateListFriends }) {
    ...
    const [statusSentReq, setStatusSentReq] = useState(false);
    const { user } = useSelector(
        (state) => state.auth
    );
    ...
    function handleClickReqFr(userId){
        let status = statusSentReq;
        setStatusSentReq(!statusSentReq);
        if(status){
            //del_req
            userService.delRequestFriend(userId).then((res) => {
                console.log(res);
            }).catch((e) => {
                console.log(e.response.data);
            })
            ...
            //sent_req
            userService.setRequestFriend(userId).then((res) => {
                console.log(res);
            }).catch((e) => {
                console.log(e.response.data);
            })
        ...
        <View style={{ justifyContent: 'center', marginLeft: 10, flex: 1 }}>
            <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                <View style={{ flexDirection: 'column', flex: 1, marginRight: 10 }}>
                    ...
                    <TouchableOpacity onPress={() => handleClickReqFr(friendData?.id)}>
                        {statusSentReq ? (
                            <Text style={{
                                paddingHorizontal: 0,
                                marginTop: 5,
